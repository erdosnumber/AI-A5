...
class MCTS
{
    ...
    MCTS(const Board& b,double weight,double iters,double sims_limit,double sims_count)
    {
        Node node(b,0,0);
        this->root=0;
        ...
        this->weight=weight;
        this->iters=iters;
        this->sims_limit=sims_limit;
        this->sims_count=sims_count;
    }
    ...
    double simulate(int node_ptr)
    {
        ...
        Board b(this->all_nodes[node_ptr].board);
        while(simulations<this->sims_limit)
        {
    ...
    void iterate()
    {
        ...
        double reward=0;
        for(int i=0;i<this->sims_count;i++)
        {
           reward+=this->simulate(expansion_node);
        }
        reward/=this->sims_count;
